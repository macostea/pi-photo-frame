...
impl Geocoder {
    ...
    pub async fn reverse_geocode(&self, latitude_dec: f32, longitude_dec: f32) -> Result<String, String> {
        let url = self.mapbox_reverse_geocoder_url.replace("{lat_dec}", latitude_dec.to_string().as_str()).replace("{lon_dec}", longitude_dec.to_string().as_str());
        let resp: MapboxResponse = reqwest::blocking::get(url).map_err(|e| e.to_string())?
            .json()
