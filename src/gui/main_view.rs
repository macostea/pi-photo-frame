...
use gtk::{
    gdk_pixbuf::Pixbuf, prelude::*, Application, ApplicationWindow, Box, Button, Image, Label,
    Overlay, Picture,
};
...
impl MainView {
    pub fn build_main_view(&mut self, app: &Application) {
        ...
        self.location_label = Some(Label::builder().halign(gtk::Align::End).build());

        self.location_label
            .as_ref()
            .unwrap()
            .add_css_class("location-label");

        self.photo_date_label = Some(Label::builder().halign(gtk::Align::End).build());

        self.photo_date_label
            .as_ref()
            .unwrap()
            .add_css_class("photo-date-label");

        self.location_box = Some(
            Box::builder()
                .halign(gtk::Align::End)
        ...
        self.location_box
            .as_ref()
            .unwrap()
            .add_css_class("location-container");

        self.location_box
            .as_ref()
            .unwrap()
            .append(self.location_label.as_ref().unwrap());
        self.location_box
            .as_ref()
            .unwrap()
            .append(self.photo_date_label.as_ref().unwrap());
        ...
        self.photo_location_label = Some(Label::builder().halign(gtk::Align::Start).build());

        self.photo_location_label
            .as_ref()
            .unwrap()
            .add_css_class("photo-location-label");
        self.photo_location_label.as_ref().unwrap().hide();

        self.play_pause_box = Some(
            Box::builder()
                .valign(gtk::Align::Start)
                .halign(gtk::Align::Start)
                .orientation(gtk::Orientation::Vertical)
        ...
        self.play_pause_box
            .as_ref()
            .unwrap()
            .add_css_class("paused-container");
        self.play_pause_box
            .as_ref()
            .unwrap()
            .append(self.play_pause_button.as_ref().unwrap());
        self.play_pause_box
            .as_ref()
            .unwrap()
            .append(self.photo_location_label.as_ref().unwrap());
        ...
        self.overlay
            .as_ref()
            .unwrap()
            .add_overlay(self.location_box.as_ref().unwrap());
        self.overlay
            .as_ref()
            .unwrap()
            .add_overlay(self.play_pause_box.as_ref().unwrap());
